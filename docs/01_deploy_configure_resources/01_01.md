...
- [Microsoft Foundry Agent Service environment setup](https://learn.microsoft.com/azure/foundry/agents/environment-setup)
- [Microsoft Foundry Agent Service quotas, limits, and regional support](https://learn.microsoft.com/azure/foundry/agents/concepts/limits-quotas-regions)
...
For the full workshop agenda, confirm the following access before the session begins:

- Azure permissions to create or use a Microsoft Foundry resource and project, model deployments, Azure AI Search, Cosmos DB, Application Insights, Storage, Azure Container Registry, Azure Container Apps, and role assignments.
- Model quota for the chat, embedding, and optional Phi model deployments used by the lab.
- Permission to assign **Azure AI User**, **Storage Blob Data Contributor**, and data-plane roles to the user and managed identities used by the Foundry project and connected tools.
- A GitHub account that can fork the repository and configure GitHub Actions repository secrets for the deployment workflow tasks.
- Optional access to Microsoft 365 Copilot, Copilot Studio, Azure API Management, and Logic Apps connectors if your session includes the low-code, AI Gateway, and custom MCP tool topics from the broader workshop agenda.
