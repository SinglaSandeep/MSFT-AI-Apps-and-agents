...
- [Python Agent Framework demos and patterns](https://github.com/Azure-Samples/python-agentframework-demos/tree/main/examples)
...
{: .note }
> In Exercise 02, you created a series of agents in the Microsoft Foundry Agent Service. In this exercise, you have created similar agents using the Microsoft Agent Framework library and integrated them into the Zava Product Manager application. While both approaches are valid, the Agent Service provides additional features such as built-in observability, monitoring, and management capabilities that are not available when using Agent Framework alone. For more information on why you might wish to use the Agent Service, review the [Agent Service documentation](https://learn.microsoft.com/azure/foundry/agents/overview#why-use-azure-ai-foundry-agent-service).

{: .note }
> For a larger interoperability reference, review the [multi-agent travel planner A2A interoperability sample](https://github.com/zhuohanl/multi-agent-travel-planner-a2a-interop). It shows how code-first agents can be exposed through A2A and adapted across Microsoft Foundry, Copilot Studio, and pro-code orchestration patterns.
